...
public class Hammer : Area2D {
	...
	private Godot.Collections.Dictionary upgrades;

	public void Init(Node2D source, float phaseShift, Godot.Collections.Dictionary upgrades = null) {
		radius = 2f;
		this.source = source;
		...
		// Duplicate the upgrades param into this upgrades, keeping integrity of any inner arrays and dictionaries.
	...
	// When a body enters the hammer Area2D
	// pass the damage the hammer deals, the source of what spawned the hammer, and all the upgrades of the hammer i.e. explode on kill
